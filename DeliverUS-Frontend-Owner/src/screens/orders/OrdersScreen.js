...
export default function OrdersScreen ({ navigation, route }) {
  ...
  const fetchRestaurantAnalytics = async () => {
    try {
      ...
      showMessage({
        message: `There was an error while retrieving restaurant analytics (id ${restaurant.id}). ${error}`,
        type: 'error',
  ...
  const fetchRestaurantOrders = async () => {
    try {
      const fetchedRestaurantOrders = await getRestaurantOrders(route.params.id)
      setOrder(fetchedRestaurantOrders)
    } catch (error) {
      showMessage({
  ...
  const handleNextStatus = async (order) => {
    try {
      ...
      showMessage({
        message: `There was an error while changing status. ${error} `,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
